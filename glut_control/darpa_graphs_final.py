...
In [28]: self.graph_rep = graph_representation(self.subjects, self.max_gnn_nodes)
     ... self.current_model.model = keras.models.load_model(os.path.join(model_prefix, "rpb_dqn_current_model_{}".format(id_str)))
     ... self.target_model.model = keras.models.load_model(os.path.join(model_prefix, "rpb_dqn_target_model_{}".format(id_str)))

In [29]: kb = 'qlearning1.pl'

In [30]: alma_inst, res = alma.init(1,kb, '0', 1, 1000, [], [])
subj_list_len 0
PyList_Size(subject_priority_list) 0

In [31]: res = rl_utils.play_episode(network, alma_inst, 10)
