...
class rl_transformer(res_prebuffer):
    def __init__(self, max_reward, reward_fn,
        ...
        if use_state:
            self.current_model = transformer_model_zero(self.max_gnn_nodes,self.graph_rep.feature_len, True,
                                                self.debugging, pytorch=self.pytorch)
            self.target_model =  transformer_model_zero(self.max_gnn_nodes,self.graph_rep.feature_len, True,
                                                self.debugging, pytorch=self.pytorch)
        else:
            self.current_model = self.model
            self.target_model =  transformer_model_zero(self.max_gnn_nodes,self.graph_rep.feature_len,
                                                self.debugging, pytorch=self.pytorch)
